...
class RL():
    def __init__(self):
        ...
        loadReplay("/home/burak/catkin_ws/src/offb/src/RLağırlık",self.buffer)
        self.att = AttitudeTarget()
        self.att_setpoint_pub = rospy.Publisher('uav0/mavros/setpoint_raw/attitude', AttitudeTarget, queue_size=1)


        try:
            self.td3.load("ağırlıklar_10500", "/home/burak/catkin_ws/src/offb/src/RLağırlık2")
    ...
    def TakeAction(self):
        if self.step >= self.start_timesteps:
		...
		action[2] = action[2] * 30
                ...
                action = (self.td3.select_action(state)+np.random.normal(0, self.max_action * 0.1,size = 3)).clip(-1,1)
	        action_for_buffer = np.copy(action)
	    ...
	    if (yeni_veri.isPlane == True):
		    ...
		    reward = rew1 - rew2
		    saveReplay("/home/burak/catkin_ws/src/offb/src/RLağırlık2",(state,new_state,action,reward,done),self.buffer)
		    self.buffer.add((state, new_state, action, reward, done))
